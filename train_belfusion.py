...
def train(cfg, model, train_loader, optimizer, criterion, device):
    losses_meters = {}
    model.train()
    for batch_idx, (_, _, s_emotion, _, _, _, l_emotion_, l_3dmm_, l_reference, gemma_embedding) in enumerate(tqdm(train_loader)):
        
        optimizer.zero_grad()

        prediction = model(speaker_emotion=s_emotion, gemma_embedding=gemma_embedding, listener_emotion=l_emotion_, listener_3dmm=l_3dmm_)
...
def validate(cfg, model, val_loader, criterion, device, epoch, mode):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (_, _, s_emotion, _, _, _, l_emotion_, l_3dmm_, l_reference, gemma_embedding) in enumerate(tqdm(val_loader)):
            # Expand inputs for num_preds variations
            s_emotion_orig = s_emotion.clone()
            l_emotion_orig = l_emotion_.clone()
            s_emotion = s_emotion.repeat_interleave(num_preds, dim=0)
            gemma_embedding = gemma_embedding.repeat_interleave(num_preds, dim=0)
            l_emotion_ = l_emotion_.repeat_interleave(num_preds, dim=0)
            l_3dmm_ = l_3dmm_.repeat_interleave(num_preds, dim=0)
            # Expand inputs for num_preds variations
            ...
            if mode == "predict":
                # Reshape prediction within the dictionary for the loss function
                prediction = {k: v.view(-1, num_preds, *v.shape[1:]) if v is not None else v for k, v in prediction.items()}
            ...
            speaker_emotions.append(s_emotion_orig)
            listener_emotions.append(l_emotion_orig)

        # compute metrics
        if mode == 'predict':
            prediction = torch.cat(all_predictions, dim=0)
            speaker_emotion = torch.cat(speaker_emotions, dim=0)
            listener_emotion = torch.cat(listener_emotions, dim=0)

            # Correct 4D Reshape: [dataset_size, num_preds, seq_len, feat_dim]
            # 210,750,000 / (10 * 750 * 25) = 1124 (The exact Valid dataset size)
            prediction = prediction.view(-1, num_preds, 750, 25)
...
def main():
    ...
    pprint(f, "WARNING: mode is set to {}".format(mode))
    last_epoch_stored = 0
    val_loss = 0
    val_metrics = None
    log_dict = {}
