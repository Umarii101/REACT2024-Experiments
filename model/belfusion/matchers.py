...
class BaseLatentModel(nn.Module):
    def __init__(self, cfg,
        ...
        checkpoint = torch.load(embedder_emotion_path, map_location=next(self.parameters()).device)
    ...
    def forward(self, pred, timesteps, seq_em):
        # Global Device Sync: move all inputs to model device
    ...
    def state_dict(self):
        ...
        if hasattr(self, 'gemma_bridge'):

            for k, v in self.gemma_bridge.state_dict().items():

                sd[f'gemma_bridge.{k}'] = v

        return sd
    
    def load_state_dict(self, state_dict):

        bridge_sd = {k.replace('gemma_bridge.', ''): v 

                     for k, v in state_dict.items() if k.startswith('gemma_bridge.')}

        model_sd = {k: v for k, v in state_dict.items() 

                    if not k.startswith('gemma_bridge.')}

        self.model.load_state_dict(model_sd)

        if bridge_sd and hasattr(self, 'gemma_bridge'):

            self.gemma_bridge.load_state_dict(bridge_sd)
...
class LatentMLPMatcher(BaseLatentModel):
    ...
    def __init__(self, cfg):
        ...
        # Gemma Bridge: Projects 2560 -> Latent Dim (e.g. 126 or 512)
        self.gemma_bridge = nn.Sequential(
            nn.Linear(2560, 512),
            nn.LayerNorm(512),
            nn.ReLU(),
            nn.Linear(512, 25)
    ...
    def forward_offline(self, speaker_emotion=None, listener_emotion=None, gemma_embedding=None, **kwargs):
        # Force sync sub-module device
        # Device enforcement for sub-modules
        ...
        if is_training: # sample random subwindow for each batch element
            ...
            output = self.diffusion.denoise(self, self.model, x_start, t, model_kwargs={
                "cond": speaker_emotion[:, window_start:window_end] + self.gemma_bridge.to(speaker_emotion.device)(gemma_embedding.to(speaker_emotion.device)).unsqueeze(1) if gemma_embedding is not None else speaker_emotion[:, window_start:window_end]
            })
            return output
            ...
            output = [output for output in self.diffusion.ddim_sample_loop_progressive(
                ...
                diff_batch,
                model_kwargs={
                    "cond": speaker_emotion + self.gemma_bridge.to(speaker_emotion.device)(gemma_embedding.to(speaker_emotion.device)).unsqueeze(1).repeat(speaker_emotion.shape[0] // gemma_embedding.shape[0], self.window_size, 1) if gemma_embedding is not None else speaker_emotion,
                },
                gt=listener_emotion
            )][-1] # get last output
