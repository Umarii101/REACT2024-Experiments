...
def val(args, model, val_loader, criterion, render, binarize=False):
    ...
    for batch_idx, (speaker_video_clip, speaker_audio_clip, speaker_emotion, _, listener_video_clip, _, listener_emotion, listener_3dmm, listener_references, gemma_embedding) in enumerate(tqdm(val_loader)):
        ...
        with torch.no_grad():
            gemma_embedding = gemma_embedding.cuda() if torch.cuda.is_available() else gemma_embedding
            prediction = model(speaker_video=speaker_video_clip, speaker_audio=speaker_audio_clip, speaker_emotion=speaker_emotion, listener_emotion=listener_emotion, gemma_embedding=gemma_embedding)
    ...
    for i in range(9):
        listener_emotion_pred_list = []
        for batch_idx, (
        speaker_video_clip, speaker_audio_clip, speaker_emotion, _, _, _, listener_emotion, _, _, gemma_embedding) in enumerate(tqdm(val_loader)):
            if torch.cuda.is_available():
                if len(speaker_video_clip.shape) != 1: # if loaded
                ...
                gemma_embedding = gemma_embedding.cuda()
            with torch.no_grad():
                gemma_embedding = gemma_embedding.cuda() if torch.cuda.is_available() else gemma_embedding
                prediction = model(speaker_video=speaker_video_clip, speaker_audio=speaker_audio_clip, speaker_emotion=speaker_emotion, listener_emotion=listener_emotion, gemma_embedding=gemma_embedding)
                if isinstance(prediction, list): # Trans VAE
...
def main(args):
    ...
    if not os.path.exists(config_path): # args-based loading --> Trans-VAE by default
        val_loader = get_dataloader(args, args.split, load_audio=False, load_video_s=False, load_video_l=False, load_emotion_s=True, load_emotion_l=True, load_3dmm_l=True, load_ref=False)
        ...
        val_loader = get_dataloader(dataset_cfg, args.split, load_audio=False, load_video_s=False, load_video_l=False, load_emotion_s=True,
                                                    load_emotion_l=True, load_3dmm_s=False, load_3dmm_l=True, load_ref=False)
